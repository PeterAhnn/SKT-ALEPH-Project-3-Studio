return await (async()=>{
  ...
  const wait=async()=>{for(let i=0;i<700;i++){if(!$('download').disabled&&!$('save-status').textContent.includes('저장 중')&&!$('save-status').textContent.includes('대기 중'))return;await new Promise(r=>setTimeout(r,25))}throw Error('busy timeout')};
  ...
  const snapshot=async()=>({saved:JSON.stringify(await readWorkspace()),canvas:$('canvas').toDataURL(),headline:$('headline').value});
  const same=async before=>JSON.stringify(await snapshot())===JSON.stringify(before);
  const oldSet=IDBObjectStore.prototype.put;
  try{
    ...
    const raw=JSON.stringify(next),before=await snapshot();
    check('백업은 내 템플릿 영역에 위치',!!$('backup').closest('.library')&&!document.querySelector('.topbar #backup'));
    await upload(raw);
    check('선택만 해서는 기존 작업 변경 없음',!$('restore-review').hidden&&(await same(before)));
    ...
    $('restore-cancel').click();check('취소 시 기존 작업 보존',(await same(before))&&$('restore-review').hidden&&document.activeElement===$('restore'));
    await upload(raw);await upload('{broken');
    check('손상 JSON은 이전 복원 후보도 해제',(await same(before))&&$('restore-review').hidden&&$('backup-status').classList.contains('error')&&$('backup-status').textContent.includes('유지'));
    $('restore-apply').click();await wait();check('거부 뒤 오래된 후보 적용 불가',(await same(before)));
    const invalid=structuredClone(next);delete invalid.templates[0].name;await upload(JSON.stringify(invalid));
    check('필수 항목 누락 시 기존 작업 보존',(await same(before))&&$('backup-status').classList.contains('error')&&$('restore-review').hidden);
    await upload(raw);IDBObjectStore.prototype.put=function(value,key){if(this.name==='workspace')throw new DOMException('full','QuotaExceededError');return oldSet.call(this,value,key)};
    $('restore-apply').click();await wait();IDBObjectStore.prototype.put=oldSet;
    check('저장 공간 오류 시 작업·목록 보존',(await same(before))&&!$('restore-review').hidden&&$('backup-status').classList.contains('error'));
    $('restore-apply').click();await wait();
    check('명시적 적용 후 전체 복원',JSON.stringify(await readWorkspace())===raw&&$('headline').value===next.current.headline&&$('restore-review').hidden);
  ...
  }catch(e){return {at:new Date().toISOString(),url:location.href,results,error:e.message}}finally{IDBObjectStore.prototype.put=oldSet}
})()
