return await (async()=>{
  ...
  const wait=async()=>{for(let i=0;i<200;i++){if(!$('download').disabled)return;await new Promise(r=>setTimeout(r,25))}throw new Error('busy timeout')};
  ...
  try {
    ...
    ]){input('headline',text);const png=await new Promise(r=>c.toBlob(r));const im=await decode(png);assert(label,im.width===1080&&im.height===1080&&JSON.parse(localStorage.getItem(KEY)).current.headline===text,`문구 ${text.length}자 보존, PNG 디코딩 성공`)}
    ...
    for(const [label,w,h,type,alpha] of [['E07 세로 PNG',400,1200,'image/png',false],['E08 가로 JPEG',1600,400,'image/jpeg',false],['E09 투명 PNG',600,600,'image/png',true]]){
      await upload(await fixture(w,h,type,alpha));assert(label,JSON.parse(localStorage.getItem(KEY)).current.image?.startsWith('data:'+type)&&$('status').textContent.includes('불러왔어요'),`${w}×${h} ${type} 업로드 성공`)
    }
    ...
    ]){const before=localStorage.getItem(KEY),preview=c.toDataURL();await upload(file);assert(label,before===localStorage.getItem(KEY)&&preview===c.toDataURL()&&$('status').classList.contains('error'),$('status').textContent)}
    ...
    for(let i=1;i<=3;i++){input('headline','테스트 카드 '+i);input('template-name','템플릿 '+i);$('save-template').click()}
    let saved=JSON.parse(localStorage.getItem(KEY));assert('C17 템플릿 3개 생성',saved.templates.length===3);
    document.querySelectorAll('.template .load')[0].click();await wait();assert('C18 첫 템플릿 불러오기',$('headline').value==='테스트 카드 1');
    input('headline','수정한 첫 카드');input('template-name','수정됨');$('update-template').click();saved=JSON.parse(localStorage.getItem(KEY));assert('C19 안정된 ID로 수정',saved.templates[0].name==='수정됨'&&saved.templates[0].state.headline==='수정한 첫 카드');
