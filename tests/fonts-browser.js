return await (async()=>{
  ...
  const wait=async()=>{for(let i=0;i<700;i++){if(!$('download').disabled)return;await new Promise(r=>setTimeout(r,25))}throw Error('font timeout')};
  ...
  try {
    await choose('sans');const before=localStorage.getItem(key),beforeCanvas=canvas.toDataURL();
    window.FontFace=class{load(){return Promise.reject(Error('simulated font network failure'))}};
    await choose('pretendard');
    check('폰트 전송 실패 시 현재 글꼴·작업·미리보기 보존',$('font').value==='sans'&&localStorage.getItem(key)===before&&canvas.toDataURL()===beforeCanvas&&$('status').classList.contains('error'));
    ...
    const count=JSON.parse(localStorage.getItem(key)).templates.length;
    input('template-name','무료폰트 복원 검사');$('save-template').click();check('폰트 ID 포함 템플릿 저장',JSON.parse(localStorage.getItem(key)).templates.at(-1).state.font==='nanum-pen');
